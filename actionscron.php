...
if($actie_brander=='yes') {
	if($switchstatus12=='off' && ($switchstatus6>$thermometerte4+0.3 || $switchstatus6>$thermometerte4+0.3)) {schakel(12, 'on', 'c');sleep(5);schakel(12, 'on', 'd');sleep(5);}
	if($switchstatus12=='on' && $switchstatus6<$thermometerte4+0.3 && $switchstatus6<$thermometerte4+0.3) {schakel(12, 'off', 'c');sleep(5);schakel(12, 'off', 'd');sleep(5);}
}
...
if($actie_timer_living=='yes'){
	if(time()>(strtotime('18:00')-(($tempw-$thermometerte1)*(($tempw-$thermometerte1)*10))) && (time()<(strtotime('22:00'))) && ($switchstatus14<$tempw || $switchstatus15<$tempw) && in_array(date('N', time()), array(1,2,3,4))) {
		...
		if($switchstatus14>$tempk || $switchstatus15>$tempk) {
			...
			if($laatsteschakel['timestamp']<(time()-7200))  {
				radiator(14, $tempk, 'c');sleep(2);
				radiator(15, $tempk, 'c');sleep(2);
			}
		}
	}
} else {
	if($switchstatus14>$tempk || $switchstatus15>$tempk) {
		...
		$laatsteschakel2 = laatsteschakeltijd(15,null, 'm');
		if($laatsteschakel2>$laatsteschakel) $laatsteschakel = $laatsteschakel2;
		if($laatsteschakel['timestamp']<(time()-7200))  {radiator(14, $tempk, 'c');sleep(2);radiator(15, $tempk, 'c');sleep(2);}
	}
}
...
if($actie_timer_badkamer=='yes'){
	if((time()>(strtotime('6:00')-(($tempw-$thermometerte4)*(($tempw-$thermometerte1)*10)))) && (time()<(strtotime('8:00'))) && ($switchstatus6<$tempw) && in_array(date('N', time()), array(1,2,3,4,5))) {
		radiator(6, $tempw, 'c');sleep(2);
	} else {
		if($switchstatus6>$tempk) {
			$laatsteschakel = laatsteschakeltijd(6,null, 'm');
			if($laatsteschakel['timestamp']<(time()-7200)) {radiator(6, $tempk, 'c');sleep(2);}
		}
	}
} else {
	if($switchstatus6>$tempk) {
...
if($actie_timer_slaapkamer=='yes'){
	$tempw = 18;
	$tempk = 8;
	if(time()>(strtotime('22:50')-(($tempw-$thermometerte4)*(($tempw-$thermometerte1)*10))) && (time()<(strtotime('23:00'))) && ($switchstatus7<$tempw)) {radiator(7, $tempw, 'c');sleep(2);
	} else {
		if($switchstatus7>$tempk) {
			$laatsteschakel = laatsteschakeltijd(7,null, 'm');
			if($laatsteschakel['timestamp']<(time()-7200))  {radiator(7, $tempk, 'c');sleep(2);}
		}
	}
} else {
	if($switchstatus7>$tempk) {
...
if($actie_timer_slaapkamertobi=='yes'){
	if(time()>(strtotime('21:20')-(($tempw-$thermometerte4)*(($tempw-$thermometerte1)*10))) && (time()<(strtotime('21:30'))) && ($switchstatus7<$tempw) && ((in_array(date('N', time()), array(5,6)) && date('W', time()) %2 == 0) || (in_array(date('N', time()), array(3,4))))) {
		...
		if($switchstatus8>$tempk) {
			...
			if($laatsteschakel['timestamp']<(time()-7200))  {radiator(8, $tempk, 'c');sleep(2);}
		}
	}
} else {
	if($switchstatus8>$tempk) {
